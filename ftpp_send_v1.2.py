...
EXCLUDED_DIRS = r".vscode, .git"  # Исключаемые директории относительно скрипта # data , data/data
...
def log(msg_type, message):
    ...
    color_codes = {
        "DEBUG": "\033[94m",  # Синий - используется для отладочной информации
        "INFO": "\033[0m",  # Стандартный - используется для основных информационных сообщений
        "WARNING": "\033[93m",  # Желтый - используется для предупреждений
        "ERROR": "\033[91m",  # Красный - используется для критических ошибок
        "SUCCESS": "\033[92m",  # Зеленый - используется для успешных операций
        "SKIPPED": "\033[90m",  # Серый - используется для пропущенных элементов
        "CREATED": "\033[96m",  # Голубой - используется для созданных объектов
    }
